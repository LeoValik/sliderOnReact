...
class Slider extends React.Component {
	...
	//Метод отвечает за то, что что-то выполнится, когда элемент появится на странице
	componentDidMount(){
		this.interval = setInterval(() => this.intervalBetweenSlides(), 3000 )
	}
	//Метод выполнится тогда, когда слайдер (или любой другой компонент) исчезнет со страницы
	componentWillUnmount(){
		clearInterval(this.interval)
	}
	//Метод автопереключения слайдов
	...
	//Обработчики событий при кликах на стрелки
	nextOne(){
	...
	//Метод проверки на активную точку
	isActive(value) {
